...
def getEmbedding(infile_path="embedding"):
    ...
    row_index = 0
    with codecs.open(infile_path, encoding="utf-8") as infile:
        for row in infile:
            row = row.strip()
            row_index += 1
            if row_index == 1:
...
def extractEntity(sentence, labels):
    entities = []
    while ('<WH>' in labels) or ('<TRG>' in labels) or ('<OBJ>' in labels):
        if '<WH>' in labels:
            index = labels.index('<WH>')
            entities.append(sentence[index])
            labels[index] = 'O'
        if '<TRG>' in labels:    
            index = labels.index('<TRG>')
            entities.append(sentence[index])
            labels[index] = 'O'
        if '<OBJ>' in labels:
...
def loadMap(token2id_filepath):
    ...
    token2id = {}
    id2token = {}
    with codecs.open(token2id_filepath, encoding="utf-8") as infile:
        for row in infile:
            row = row.rstrip()
            token = row.split('\t')[0]
...
def saveMap(id2char, id2label):
    with codecs.open("char2id", mode="w", encoding="utf-8") as outfile:
        for idx in id2char:
            outfile.write(id2char[idx] + "\t" + str(idx) + "\r\n")
    with codecs.open("label2id", mode="w", encoding="utf-8") as outfile:
        for idx in id2label:
...
def getTrain(train_path, val_path, train_val_ratio=0.99, use_custom_val=False, seq_max_len=40):
...
def getTest(test_path="test.in", is_validation=False, seq_max_len=40):

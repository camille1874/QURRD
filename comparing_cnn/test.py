    ...
    for e in range(1, epoch + 1):
        ...
        with tf.Session(config=session_config) as sess:       
            ...
            for i in range(test_data.data_size):
                pred, estimation, clf_input = sess.run([model.prediction, model.estimation, model.output_features],
                ...
                #pred = estimation[0][1] - estimation[0][0]
                if classifier == "LR":
                ...
                if s1 in QA_pairs:
                    if i != 0 and test_data.s1s[i - 1] == test_data.s1s[i]:
                        if s1 + "1" not in QA_pairs:
                            QA_pairs[s1].append((s2, labels[i], np.asscalar(pred)))                 
                        else:
                            i = 1
                            while s1 + str(i) in QA_pairs:
                                i += 1
                            QA_pairs[s1 + str(i - 1)].append((s2, labels[i], np.asscalar(pred)))
                    else:
                        i = 1
                        while s1 + str(i) in QA_pairs:
                            i += 1
                        QA_pairs[s1 + str(i)] = [(s2, labels[i], np.asscalar(pred))]
                else:

...
class WebQSP(Data):
    def open_file(self, mode):
        if mode == "train":
            ...
            f = "all-test"
        with codecs.open("./WebQSP_Corpus/WebQSP" + f + ".txt", "r", encoding="utf-8") as f:
...
class SQ(Data):
    def open_file(self, mode):
        if mode == "train":
            ...
            f = "all-test"
        with codecs.open("./SQ_Corpus/SQ" + f + ".txt", "r", encoding="utf-8") as f:
            stopwords = nltk.corpus.stopwords.words("english")
            for line in f:
                items = line[:-1].split("\t")
                s1 = clean_str(items[0]).split()[:28]
                s2 = clean_str(items[1]).split()[:28]
                label = int(items[2])

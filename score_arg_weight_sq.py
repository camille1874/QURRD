...
import codecs
import math
score1 = codecs.open("20180507_084330prediction.csv", encoding="utf-8").readlines()
score2 = codecs.open("result_sq44", encoding="utf-8").readlines()
#final_f = codecs.open("result_63_sq.txt", encoding="utf-8", mode="w")
y = codecs.open("./completing_cnn/data/sqdata/SQ.final_test_y_raw.txt").readlines()
y = [int(t.strip()) for t in y]
scores2 = [float(t.strip().split("  ")[1].split()[-1]) for t in score2]
        ...
        for a in range(0, 20):
            ...
            for i, l in enumerate(score2):
                ...
                if s1 < 0 and s2 < 0:
                    ...
                    s2 = s2 * w 
                s = a * s1 + b * s2
                if (i != 0 and y[i] == 1) or i == len(score2) - 1:
